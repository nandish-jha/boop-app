...
function detectType(lower: string): CaptureType {
  ...
  // If it contains a time reference and an action verb, lean task
...
export function extractDueDate(lower: string): number | null {
  ...
  for (const [word, offset] of Object.entries(dayMap)) {
    ...
    if (word === 'next week') {
      baseDate = new Date(todayMidnight); baseDate.setDate(baseDate.getDate() + 7)
      break
    }
    if (word === 'next month') {
      baseDate = new Date(todayMidnight); baseDate.setDate(baseDate.getDate() + 30)
      break
    }
    if (dayNames.includes(word.replace('next ', ''))) {
      ...
      baseDate = new Date(todayMidnight); baseDate.setDate(baseDate.getDate() + diff)
      break
    }
    baseDate = new Date(todayMidnight); baseDate.setDate(baseDate.getDate() + offset)
    break
  }

  // Time extraction: "at 2pm", "at 3:30", "2 pm", "14:00"
  ...
  // If we found a day keyword but no time, default to 9am
...
const AREA_PATTERNS: Array<[RegExp, string]> = [
  [/\b(home|house|apartment|flat|kitchen|bathroom|garage|garden|yard|bedroom|living room)\b/, 'home'],
  [/\b(work|office|job|client|meeting|project|boss|colleague|team|slack|zoom|call)\b/, 'work'],
  [/\b(health|doctor|gym|exercise|workout|run|jog|medication|medicine|dentist|hospital|clinic)\b/, 'health'],
  [/\b(finance|money|bill|bank|pay|rent|insurance|tax|budget|invoice|expense)\b/, 'finance'],
  [/\b(school|study|class|lecture|exam|homework|assignment|university|college)\b/, 'personal'],
  [/\b(family|kid|son|daughter|wife|husband|parent|mom|dad|birthday|anniversary)\b/, 'personal'],
]
...
const FILLER_PATTERNS = [
  ...
  /\badd (a |an )?(task|reminder|note) (for |to )?(me )?/gi,
...
function extractTitle(cleaned: string, type: CaptureType): string {
  // Remove time/date suffix for the title
  let title = cleaned
  ...
  return title || (type === 'TASK' ? 'New task' : 'New note')
}
